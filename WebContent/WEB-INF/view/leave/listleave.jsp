...
<section class="content-header">
    ...
    <div class="col-sm-4">
		...
		<button id="addleave" type="button" class="btn btn-info"><span class="glyphicon glyphicon-plus"></span>请假申请</button></a>
	</div>
	...
	$(function(){
		...
		$.post("<%=request.getContextPath()%>/getLeaveListPage.json",ipage,function(data){
			$("#page-div").createPage({
	...
	function tbodyInit(data){
		...
		$.each(data,function(i,o){
				htmlstr += "<tr>"
					...
					+	"<td>"+o.leaveApplyTime+"</td>"
					+	"<td>"+o.leaveSum+"</td>"
					+	"<td class =\""+o.leaveState+"\"/>"
					+	"<td><a href='javascript:void(0);' onclick='update(\""+o.leaveNo+"\");' >修改</a></td>"
					+	"<td><a href='javascript:void(0);' onclick='delet(\""+o.leaveNo+"\");' >删除</a></td></tr>"
					
					 selectdicValue(o.leaveType);
					 selectdicValue(o.leaveState);  // 用数据字典进行替换
	...
	$("#search-btn").click(function(){
		...
		$.post(contextPath+"/getLeaveListPage.json",ipage,function(data){
			$("#page-div").createPage({
		        pageCount:data.ipage.totalPage,
		        current:1,
		        backFn:function(p){
	...
	function delet(leaveNo){
		layer.confirm('确定要删除此请假吗？', {
			  btn: ['确定','返回'] //按钮
			}, function(){
				$.ajax({
					url:'<%=request.getContextPath()%>/deleteLeave.json',
	...
	function update(leaveNo){
		return window.location.href="leaveInfo?leaveNo="+leaveNo;
	}
